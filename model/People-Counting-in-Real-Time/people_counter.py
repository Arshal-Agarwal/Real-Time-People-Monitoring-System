...
def notify_server(count):
    """Send count updates to the server"""
    try:
        response = requests.post(
            'https://mobile-alert-backend.onrender.com/api/notify',
            json={'count': count},
            headers={'Content-Type': 'application/json'}
        )
        if response.status_code == 200:
            logger.info(f"Server notified. Current count: {count}")
        else:
            logger.error(f"Failed to notify server. Status code: {response.status_code}")
